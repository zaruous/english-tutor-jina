...
## 0. ~~열린 PR 4개 머지~~ → 2026-09-13 완료 · 남은 것은 PR #13 처분

PR #10 → #11 → #12 를 순서대로 main 에 머지했다(`a50540b` · `5e54768` · `580f793`, merge commit 방식).
GitHub 이 스택 PR 의 base 를 **자동으로 바꿔 주지 않았다**(브랜치를 지우지 않아서) — 각 머지 직전 `gh pr edit N --base main` 을 직접 실행해야 했다.
머지 뒤 main 에서 `npm test` 125/125.

- **PR #13 `claude/implementation-checklist-review-k1fvii`(DRAFT)** — 다른 클라우드 Claude 세션이 2026-09-06 에 main(3ac85ba)에서 갈라져
  플랜 11+12+13 A·B+10-3(발음 이력)을 **병렬로 다시 구현**한 브랜치(5커밋, 46파일 +4092). 로컬 체인 머지 후 `CONFLICTING`(23파일, add/add 다수).
  사용자 결정: 로컬 체인이 정본, #13 은 그대로 둠. **#13 에만 있는 자산** = `scripts/fixtures/ko-misread-8s.wav`(한국어 오독 8초 픽스처) ·
  `scripts/mock-pronounce-sidecar.mjs` · `e2e:speaking-assess` — §2 플랜 10 실측 때 선별해 가져올 재료. 닫을지 선별 반영할지 미결.
