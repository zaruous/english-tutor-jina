...
phases:
  ...
  - { id: B, name: "토픽 생성 · 구성 · 순서", status: done, done_at: 2026-09-06, note: "admin-topic.service + /api/admin/topics (목록·생성·수정·PUT contents 일괄 저장·status/visibility — 전이는 콘텐츠와 같은 canTransition). admin '토픽' 탭 — 목록(eligible 경고 배지) + 컴포저(↑↓ 순서·제거·검색 추가). 구성은 콘텐츠 상태와 무관, 학습자에게는 공개된 것만(topic.service discoverable). 토픽 감사 로그는 v1 없음 — content_audit_log 가 content FK 라 기록 불가(후속). tests/admin-topic.service.test.mjs · e2e-admin-contents 30/30" }
...
- **토픽 구성은 그대로.** 11 결정 3 으로 빈 토픽이 관리자에게 보이게 됐으니, 토픽 만들기 + 콘텐츠 붙이기가
  카탈로그를 실제로 채우는 마지막 조각이다.

## 화면 미리보기

규칙은 [11 의 목업 절](11-content-lifecycle-admin.md#화면-미리보기--관리자-콘텐츠-시리즈-4화면).

| 화면 | Phase | 미리보기 | 목업 소스 |
|---|---|---|---|
| LC 에디터(최소형) | A | [`img/13-lc-editor.png`](img/13-lc-editor.png) | [`mockups/13-lc-editor.html`](mockups/13-lc-editor.html) |
| 토픽 구성 | B | [`img/13-topic-composer.png`](img/13-topic-composer.png) | [`mockups/13-topic-composer.html`](mockups/13-topic-composer.html) |

### LC 에디터 (Phase A)
![LC 에디터](img/13-lc-editor.png)

읽을 것: 화면 전체가 **AI 초안을 고치는** 모양이다 — 새로 만들기 버튼이 없고 제목·kind 가 이미 채워져 있다.
하단 빨간 띠가 이 화면의 핵심이다: 검증 규칙의 단일 소스는 서버이고(결정 2), 화면은 돌려받은
`validation_errors` 를 **그대로 렌더할 뿐** 클라이언트에서 다시 판단하지 않는다. 그림의 3번 줄은
화자가 지정되지 않아 저장이 거부된 상태다.

> ⚠ **스크립트 편집 방식이 13 과 10.7 사이에서 어긋나 있다 (열린 질문 5).** 이 목업은 화자를
> `M`/`W` **토글**로 그렸다 — [10.7 §3.2](10.7-db-rebaseline.md) 가 `passage.body` 를
> `[{speaker, text}]` 로 구조화하기 때문이다. 반면 아래 Phase A 표는 `M:`/`W:` 접두를 텍스트로 그대로 두는
> 줄 단위 textarea 라고 적고 있다. 둘 중 하나를 골라야 한다.

### 토픽 구성 (Phase B)
![토픽 구성](img/13-topic-composer.png)

읽을 것: `eligible` 미달(레슨 1/3)인데도 **저장·공개가 막히지 않는다** — 임계치가 필터에서 경고로
격하된 결과다(11 결정 3). 붙인 콘텐츠에 `검토` 상태 항목이 섞여 있는 것도 의도적이다: 구성은 상태와
무관하게 짜두고 공개는 각 콘텐츠의 전이로 따로 간다. 탭에 **스피킹이 없는 것**이 Phase C 게이트의 표시다.

## 1. 설계 결정

1. **저작은 같은 테이블에 쓴다.** 새 콘텐츠 엔진 없음(원문 "먼저 하지 말 것"). 레슨은 `lessons`+`lesson_items`,
   시나리오·단어 세트는 본 테이블. 생성은 `status='draft'`(11 결정 1 의 기본값과 일치), 공개는 11 의 전이로.
   **저작 권한은 `author` 이상**(11 결정 6) — 에디터는 만들고 고치고 `draft → review` 까지 할 수 있고,
   자기가 쓴 것을 스스로 공개하지는 못한다. `reviewer` 이상이면 같은 화면에서 발행까지 이어진다.
...
| `src/admin/editors/topic.jsx` | 토픽 만들기 + 콘텐츠 붙이기(검색·선택)/순서 + `eligible` 경고 배지(11 결정 3) |
...
| 산출물 | 세부 |
|---|---|
| `0018_speaking_set_details.sql` | 아래 §3 — detail 테이블 **하나**. `topic_contents` 는 10.7 의 `(topic_id, content_id)` 단일 FK 라 손대지 않는다 |
...
5. ~~**LC 스크립트 편집 단위**~~ → **화자 토글 + 본문 필드로 확정** (2026-09-05, Phase A 구현).
   ...
   **LC 스크립트 편집 단위 — 화자 토글인가, `M:` 접두 텍스트인가 (2026-09-03 검토).**
   ...
   이유(정규식으로 라벨을 떼는 코드를 없앤다)를 화면에서 되돌리는 셈이다.
   **화자 토글 + 본문 필드**가 스키마와 맞고 목업도 그렇게 그렸다. 다만 줄을 통째로 붙여넣는 편집은 불편해지므로,
   원안대로 갈 거면 10.7 의 LC 구조화를 함께 재검토해야 한다.
