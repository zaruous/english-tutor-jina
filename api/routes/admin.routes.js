...
// 저작 에디터가 있는 유형 (플랜 13 Phase A 레슨 → 플랜 14 Phase D 회화·단어). speaking_set 은 에디터가 없어
// 400 이다(13 Phase C 게이트). `:type` → 서비스 함수 표로 분기하고 응답 키는 서비스가 돌려주는 그대로다
// ({ lesson } · { scenario } · { vocab_set }) — 기존 테스트·화면이 { lesson } 을 단정하므로 공통 키로 뭉개지 않는다.
const AUTHORING = Object.freeze({
  lesson: { read: adminAuthoring.readLesson, create: adminAuthoring.createLesson, update: adminAuthoring.updateLesson },
  scenario: { read: adminAuthoring.readScenario, create: adminAuthoring.createScenario, update: adminAuthoring.updateScenario },
  vocab_set: { read: adminAuthoring.readVocabSet, create: adminAuthoring.createVocabSet, update: adminAuthoring.updateVocabSet },
});
const AUTHORING_TYPES = Object.freeze(Object.keys(AUTHORING));
...
export function registerAdminRoutes(router) {
  ...
  // ── 저작 — 읽기·생성·수정 (플랜 13 Phase A · 설계 검토 D2·D3·D6 → 플랜 14 Phase D 유형 3종) ──────
  // 권한은 author 이상 하나로 끝난다 — 상태를 바꾸지 않는 조작이라 전이표가 개입할 자리가 없다.
  // 상세는 학습자 DTO 와 달리 레슨의 answer·explanation, 회화의 system_prompt 를 싣는다 — 그래서 이 세 경로만
  // 그것을 내보내고 학습자 라우트는 절대 넓히지 않는다. `:type` 은 허용값 밖이면 400 이고(oneOf 가 먼저), 행의 실제
  // type 이 다른 id 는 서비스가 404 로 뭉갠다(…/status 와 같은 순서 — 다른 유형 id 탐색 통로를 열지 않는다).
  router.get('/api/admin/contents/:type/:id', async (req, res, { params }) => {
    await requireAuthor(req, res);
    const type = oneOf(params.type, 'type', AUTHORING_TYPES);
    sendJson(res, 200, { ok: true, ...(await AUTHORING[type].read(posInt(params.id, 'id'))) });
  });
  ...
  router.post('/api/admin/contents/:type', async (req, res, { params }) => {
    ...
    const type = oneOf(params.type, 'type', AUTHORING_TYPES);
    const body = await readJson(req);
    await sendAuthoring(res, 201, () => AUTHORING[type].create(user, body));
  });

  // 수정은 본문(문항·objectives·words)을 통째로 갈아 끼우고 status·visibility 는 그대로 둔다. seed 행은 curated 로 바뀐다(결정 5).
  router.patch('/api/admin/contents/:type/:id', async (req, res, { params }) => {
    const { user } = await requireAuthor(req, res);
    const type = oneOf(params.type, 'type', AUTHORING_TYPES);
    const contentId = posInt(params.id, 'id');
    const body = await readJson(req);
    await sendAuthoring(res, 200, () => AUTHORING[type].update(user, contentId, body));
  });
}
